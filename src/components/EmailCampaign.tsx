...
export default function EmailCampaign({ 
  ...
  // MailerLite configuration state fetched from server securely
  const [mailerliteStatus, setMailerliteStatus] = useState<{ configured: boolean; senderEmail: string | null }>({
    configured: false,
    senderEmail: null
  });

  React.useEffect(() => {
    fetch('/api/mailerlite-status')
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => {
        console.error("Erreur lors de la récupération du statut MailerLite:", err);
      });
  }, []);
...
<p>Créez des scénarios transactionnels avancés de tests et gérez tout votre CRM facilement.</p>`
  ...
  const handleSend = () => {
    ...
    setTimeout(async () => {
      ...
      const isMailerliteConfigured = mailerliteStatus.configured;
      ...
      let fromEmail = mailerliteStatus.senderEmail || 'support@flashconnect.net';
      let fromName = 'Alerte Sécurisée';
      
      const maskMatch = senderMask.match(/^(.*?)\s*<(.*?)>$/);
      if (maskMatch) {
        fromName = maskMatch[1].trim();
        fromEmail = mailerliteStatus.senderEmail || maskMatch[2].trim();
      ...
      for (const rec of recipientsList) {
        ...
        // 1. Send real email via secure backend endpoint if MailerLite is configured
        if (isMailerliteConfigured) {
          try {
            ...
            if (!response.ok) {
              ...
              console.error("Secure MailerLite API Error Response:", errDetails);
              throw new Error(errDetails?.error || `Statut ${response.status}`);
            }
          } catch (err: any) {
            console.error(`Secure MailerLite dispatch failed for ${rec.email}:`, err);
            realSendError = err.message || "Erreur de connexion";
          }
        }
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {mailerliteStatus.configured ? (
                    <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full font-semibold flex items-center gap-1">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                      MailerLite Actif (Réel)
                    </span>
                  ) : (
                    <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full font-semibold flex items-center gap-1">
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {mailerliteStatus.configured 
                    ? `Vos e-mails sont réellement envoyés via MailerLite (Expéditeur : ${mailerliteStatus.senderEmail || 'Configuration Cloudflare'})`
